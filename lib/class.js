...
function define(namespace, api) {
    ...
    if(Array.isArray(config.members)) {
        ...
        for (var key in config.members) {
            var property = key;
            var options = config.members[key]; // preperation for air conditioner

            if (options.toLowerCase() != 'auto') {
                throw new Error("only 'auto' property option is supported for now");
            }
