...
function define(namespace, api) {
    ...
    if (_.has(api, 'constructor')) {
        ...
        klass = (function (supProto) {
            return config.name
                ? new Function('supProto', 'api', 'return function ' + config.name + '() {this.callParent = supProto["constructor"];return api.constructor.apply(this, arguments);}')(supProto, api)
                : function () {this.callParent = supProto["constructor"];return api.constructor.apply(this, arguments);}
        })(parent.prototype);
    }
        ...
        klass = (function (parent) {
            return config.type
                ? new Function('parent', 'return function ' + config.name + ' () {parent.apply(this, arguments); }')(parent)
                : function  () {parent.apply(this, arguments); };

        })(parent);

    } else {
        klass =  config.type ? new Function('return function ' + config.name + ' () {}')(): function () {}
    }
...
//    })(parent,klass.prototype);
    ...
    if(namespace){
       var test = util.namespace(namespace,GLOBAL,klass);
       var test = "aaa"
    }
